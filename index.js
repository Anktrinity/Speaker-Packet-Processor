...
// Middleware - Configure CORS for Replit proxy
app.use(cors({
  origin: true, // Allow all origins for Replit's proxy environment
  credentials: true,
...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (!smartTaskCreator.hasOpenAI) {
      return res.status(503).json({ 
        success: false, 
        error: 'AI chat is not available. Please configure OPENAI_API_KEY to use this feature.' 
      });
    }
...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    if (!smartTaskCreator.hasOpenAI) {
      return res.status(503).json({ 
        success: false, 
        error: 'AI analysis is not available. Please configure OPENAI_API_KEY to use this feature.' 
      });
    }
