...
class AIService {
  constructor() {
    ...
    if (this.hasOpenAI) {
      try {
        ...
        logger.warn('Failed to initialize AI Service:', error.message);
        this.hasOpenAI = false;
      }
    } else {
      logger.warn('OpenAI API key not provided - AI analysis features will be disabled');
    }
  }

  async analyze(data, type = 'general') {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI analysis features.');
    }
  ...
  async generateSuggestion(context, task) {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI suggestion features.');
    }
