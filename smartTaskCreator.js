...
class SmartTaskCreator {
  constructor() {
    ...
    if (this.hasOpenAI) {
      try {
        ...
        logger.warn('Failed to initialize OpenAI service:', error.message);
  ...
  async parseNaturalLanguageRequest(request, context = {}) {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI task creation features.');
    }
...
Respond with valid JSON only.`;
  ...
  async suggestMissingTasks() {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI task suggestion features.');
    }
...
Respond with valid JSON in the same task format.`;
  ...
  async autoAssignTasks(tasks, teamContext = {}) {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI task assignment features.');
    }
...
Respond with JSON:
{
  ...
  async generateTaskRecommendations(taskId) {
    if (!this.hasOpenAI) {
      throw new Error('OpenAI service is not available. Please provide an OPENAI_API_KEY to use AI task recommendation features.');
    }
